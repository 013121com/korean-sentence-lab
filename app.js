(function(){
  ...
  const prevIndex=(i,n)=>(i-1+n)%n;
  const nextIndex=(i,n)=>(i+1)%n;
  const sections=[
  ...
  function renderPos(){const q=posQs[posIdx];$('#posWord').textContent=q.w;$('#posHint').textContent=`${posIdx+1} / ${posQs.length} · ${q.h}`;$('#posFeedback').className='feedback';$('#posFeedback').textContent='';$('#posChoices').innerHTML=shuffle(posChoices).map(c=>`<button class="choice" data-pos="${c}">${c}</button>`).join('');$$('[data-pos]').forEach(b=>b.onclick=()=>{const ok=b.dataset.pos===q.a;$$('[data-pos]').forEach(x=>x.disabled=true);b.classList.add(ok?'correct':'wrong');const correct=$(`[data-pos="${q.a}"]`);if(correct)correct.classList.add('correct');$('#posFeedback').className='feedback show '+(ok?'ok':'no');$('#posFeedback').textContent=ok?'정답입니다. 단어 자체의 갈래를 찾았습니다.':`정답은 ${q.a}입니다.`;});}
  $('#posPrev').onclick=()=>{posIdx=prevIndex(posIdx,posQs.length);renderPos()};
  $('#posNext').onclick=()=>{posIdx=nextIndex(posIdx,posQs.length);renderPos()};renderPos();
  ...
  function renderId(){const q=idQs[idIdx];idP=idC=null;$('#idSentence').innerHTML=q.s.replace(q.t,`<mark style="background:#fff1a8;padding:0 3px;border-radius:5px">${q.t}</mark>`);$('#idTarget').textContent=`${idIdx+1} / ${idQs.length} · ‘${q.t}’의 품사와 문장 성분을 각각 고르세요.`;$('#idFeedback').className='feedback';$('#idPosChoices').innerHTML=shuffle(idPos).map(x=>`<button class="choice" data-idp="${x}">${x}</button>`).join('');$('#idCompChoices').innerHTML=shuffle(idComp).map(x=>`<button class="choice" data-idc="${x}">${x}</button>`).join('');$$('[data-idp]').forEach(b=>b.onclick=()=>{$$('[data-idp]').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');idP=b.dataset.idp});$$('[data-idc]').forEach(b=>b.onclick=()=>{$$('[data-idc]').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');idC=b.dataset.idc});}
  ...
  $('#idPrev').onclick=()=>{idIdx=prevIndex(idIdx,idQs.length);renderId()};
  $('#idNext').onclick=()=>{idIdx=nextIndex(idIdx,idQs.length);renderId()};renderId();
  ...
  $('#compPrev').onclick=()=>{compIdx=prevIndex(compIdx,compQs.length);renderComp()};
  $('#compNext').onclick=()=>{compIdx=nextIndex(compIdx,compQs.length);renderComp()};renderComp();

  const skQs=shuffle([{s:'꽃이 피었다.',a:'어찌하다'},{s:'밤바람이 차갑다.',a:'어떠하다'},{s:'나는 중학생이다.',a:'무엇이다'},{s:'새가 모이를 먹는다.',a:'어찌하다'},{s:'해미는 초등학생이 아니다.',a:'어떠하다'}]);let skIdx=0;
  function renderSk(){const q=skQs[skIdx];$('#skSentence').textContent=q.s;$('#skFeedback').className='feedback';$('#skChoices').innerHTML=shuffle(['어찌하다','어떠하다','무엇이다']).map(c=>`<button class="choice" data-sk="${c}">${c}</button>`).join('');$$('[data-sk]').forEach(b=>b.onclick=()=>{const ok=b.dataset.sk===q.a;$$('[data-sk]').forEach(x=>x.disabled=true);b.classList.add(ok?'correct':'wrong');const correct=$$('[data-sk]').find(x=>x.dataset.sk===q.a);if(correct)correct.classList.add('correct');$('#skFeedback').className='feedback show '+(ok?'ok':'no');$('#skFeedback').textContent=ok?'정답입니다. 서술어의 성격을 잘 찾았습니다.':`정답은 ${q.a}입니다.`;});}
  $('#skPrev').onclick=()=>{skIdx=prevIndex(skIdx,skQs.length);renderSk()};
  $('#skNext').onclick=()=>{skIdx=nextIndex(skIdx,skQs.length);renderSk()};renderSk();
  ...
  $('#stPrev').onclick=()=>{stIdx=prevIndex(stIdx,stQs.length);renderSt()};
  $('#stNext').onclick=()=>{stIdx=nextIndex(stIdx,stQs.length);renderSt()};renderSt();
  ...
  function renderCp(){const q=cpQs[cpIdx];$('#cpSentence').textContent=q.s;$('#cpHint').textContent=`${cpIdx+1} / ${cpQs.length} · ${q.h}`;$('#cpFeedback').className='feedback';$('#cpChoices').innerHTML=shuffle(cpChoices).map(c=>`<button class="choice" data-cp="${c}">${c}</button>`).join('');$$('[data-cp]').forEach(b=>b.onclick=()=>{const ok=b.dataset.cp===q.a;$$('[data-cp]').forEach(x=>x.disabled=true);b.classList.add(ok?'correct':'wrong');const cor=$$('[data-cp]').find(x=>x.dataset.cp===q.a);if(cor)cor.classList.add('correct');$('#cpFeedback').className='feedback show '+(ok?'ok':'no');$('#cpFeedback').textContent=ok?'정답입니다. 결합 방식과 절의 역할을 함께 확인했습니다.':`정답은 ${q.a}입니다.`;});}
  $('#cpPrev').onclick=()=>{cpIdx=prevIndex(cpIdx,cpQs.length);renderCp()};
  $('#cpNext').onclick=()=>{cpIdx=nextIndex(cpIdx,cpQs.length);renderCp()};renderCp();
